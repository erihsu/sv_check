    ...
    match t.kind {
        ...
        TokenKind::Str => {
            ...
            if t.kind==TokenKind::KwPure || t.kind==TokenKind::KwContext {
                ts.flush(1);
                n.attr.insert("property".to_string(),t.value);
            } else {ts.rewind(1);}
...
pub fn parse_port_decl(ts : &mut TokenStream, allow_void : bool) -> Result<AstNode, SvError> {
    ...
    if t.kind == TokenKind::KwConst {
        node.attr.insert("const".to_string(), "".to_string());
        t = next_t!(ts,true);
    }
    // direction/interface
    match t.kind {
        ...
        TokenKind::Ident => {
            ...
            match nt.kind {
                ...
                // Any other token is an error
                _ =>  return Err(SvError::syntax(nt, "port declaration. Expecting identifer, comma, ), #".to_string()))
            }
        }
        TokenKind::KwIntf => {
            type_found = true;
...
pub fn parse_signal_decl_list(ts : &mut TokenStream, node: &mut AstNode) -> Result<(), SvError> {
    ...
    loop {
        ...
        match nt.kind {
            ...
            _ => return Err(SvError::syntax(nt, "signal declaration, expecting , or ;".to_string()))
        }
    }
...
pub fn parse_opt_scope(ts : &mut TokenStream, node : &mut AstNode) -> Result<(), SvError> {
    ...
    n.attr.insert("name".to_string(),t_ident.value);
    ts.flush(2);
...
pub fn parse_ident_hier(ts : &mut TokenStream) -> Result<AstNode, SvError> {
    ...
    // println!("[parse_ident_hier] start: {}", t);
...
pub fn parse_enum(ts : &mut TokenStream) -> Result<AstNode,SvError> {
    ...
    match t.kind {
        ...
        TokenKind::Ident => {
            ts.flush(0);
            node_e.attr.insert("type".to_string(),t.value);
            t = next_t!(ts,false);
        }
        TokenKind::CurlyLeft => {}
...
pub fn parse_typedef(ts : &mut TokenStream, node: &mut AstNode) -> Result<(), SvError> {
    ...
    match t.kind {
        ...
        TokenKind::KwClass => {
            ts.flush(1);
            node_type = AstNode::new(AstNodeKind::Typedef);
            node_type.attr.insert("kind".to_string(),t.value);
        }
        _ => return Err(SvError::syntax(t, "typedef declaration, expecting type/enum/struct".to_string()))
    }
    // Parse type name
    t = next_t!(ts,false);
    ...
    if t.kind == TokenKind::SquareLeft {
        let ws = parse_range(ts)?;
        node_type.attr.insert("unpacked".to_string(), ws);
        t = next_t!(ts,false);
    }
    // Expec semi-colon
...
pub fn check_label(ts : &mut TokenStream, name: &str) -> Result<(), SvError> {
    ...
    if t.kind == TokenKind::Colon {
        ...
        if t.kind!=TokenKind::Ident && t.kind!=TokenKind::KwNew {
            return Err(SvError::syntax(t, "label".to_string()));
                    ...
                    if t.kind == TokenKind::Ident {
                        loop {
                            ...
                            match t.kind {
                                ...
                                _ =>  return Err(SvError::syntax(t,"define. Expecting port name/expression".to_string())),
                            }
                        }
                    } else {ts.rewind(0);}
                }
...
pub fn parse_vintf(ts : &mut TokenStream, node : &mut AstNode) -> Result<(), SvError> {
    ...
    if t.kind==TokenKind::KwIntf {
        t = next_t!(ts,false);
        node_i.attr.insert("keyword".to_string(),"explicit".to_string());
    } else {
        node_i.attr.insert("keyword".to_string(),"implicit".to_string());
    }
...
pub fn parse_expr(ts : &mut TokenStream, cntxt: ExprCntxt, allow_type: bool) -> Result<AstNode, SvError> {
    ...
    loop {
        ...
        match t.kind {
            ...
            TokenKind::SemiColon if cntxt!=ExprCntxt::StmtList && cntxt!=ExprCntxt::Stmt  => return Err(SvError::syntax(t, "expression".to_string())),
            ...
            TokenKind::Comma if cnt_c==0 => {
                if cntxt==ExprCntxt::Stmt || cntxt==ExprCntxt::Arg || cntxt==ExprCntxt::ExprGroup  {
                    return Err(SvError::syntax(t, "expression".to_string()));
                }
            ...
            TokenKind::TickCurly if is_first  => {
                ...
                loop {
                    ...
                    if is_struct {
                        t = next_t!(ts,false);
                        if t.kind != TokenKind::Ident && t.kind != TokenKind::Integer && t.kind != TokenKind::Str && t.kind != TokenKind::KwDefault {
                            return Err(SvError::syntax(t, "struct init. Expecting identifier/integer/default".to_string()));
                        }
                        s = t.value;
                        t = next_t!(ts,false);
                        if t.kind != TokenKind::Colon {
            ...
            TokenKind::ParenRight  => {
                if cnt_p == 0 {
                    match cntxt {
                        ...
                        _ => return Err(SvError::syntax(t, "expression".to_string()))
                    }
                }
                ts.flush(1);
                cnt_p -= 1
            ...
            TokenKind::KwNew if is_first => {
                ...
                if t.kind == TokenKind::SquareLeft {
                    ...
                    expect_t!(ts,"new size",TokenKind::SquareRight);
                    // ts.flush(0);
                    t = next_t!(ts,true);
                }
                if t.kind == TokenKind::ParenLeft {
                    ...
                    if t.kind != TokenKind::ParenRight {
                        ...
                        loop {
                            ...
                            match t.kind {
                                ...
                                _ => return Err(SvError::syntax(t, "new arguments. Expecting , or )".to_string()))
                            }
                        }
                    } else {
                        ts.flush(1);
                ...
                match t.kind {
                    ...
                    _ => return Err(SvError::syntax(t, "new statement.".to_string()))
                }
                ts.rewind(0);
                allow_ident = false;
                ...
                if ! allow_ident {
                    match t.value.as_ref() {
                        "fs" |"ps" |"ns" |"us" |"ms" | "s" => {
                            match prev_kind {
                                TokenKind::Integer | TokenKind::Real => {
                                    node_e.attr.insert("value".to_string(), t.value);
                                    ts.flush(1);
                                }
                                _ => return Err(SvError::syntax(t, "expression. Unexpected identifier".to_string()))
                            };
                        }
                        _ => return Err(SvError::syntax(t, "expression.".to_string()))
                    }
            ...
            TokenKind::OpIncrDecr => {
                if is_first {
                    node_e.attr.insert("incr_decr".to_string(),t.value);
                    node_e.attr.insert("op".to_string(),"pre".to_string());
                    allow_ident = true;
                }
                else {
                    node_e.attr.insert("incr_decr".to_string(),t.value);
                    node_e.attr.insert("op".to_string(),"post".to_string());
                    allow_ident = false;
                }
                ts.flush(1);
            }
            ...
            TokenKind::TypeIntVector if allow_type => {
                ...
                node_e.attr.insert("type".to_string(), t.value.clone());
                ts.flush(1);
                t = next_t!(ts,true);
                if t.kind == TokenKind::SquareLeft {
                    ts.flush(1);
                    node_e.attr.insert("unpacked".to_string(), parse_range(ts)?);
                } else {
                    ts.rewind(1);
                }
    ...
    if cnt_c != 0 {
        return Err(SvError::syntax(t, "Unbalanced curly braces".to_string()));
    }
    if cnt_p != 0 {
        return Err(SvError::syntax(t, "Unbalanced parenthesis".to_string()));
    }
    if cnt_s != 0 {
        return Err(SvError::syntax(t, "Unbalanced square bracket".to_string()));
    }
    // println!("[Expr] {}", node_e);
...
pub fn parse_member_or_call(ts : &mut TokenStream, is_first: bool) -> Result<AstNode, SvError> {
    ...
    if t.kind!=TokenKind::Ident && t.kind!=TokenKind::KwNew {
        if (t.kind != TokenKind::KwThis && t.kind != TokenKind::KwSuper) || !is_first {
            return Err(SvError::syntax(t, "member. Expecting identifier".to_string()));
        }
    }
    let name = t.value;
    t = next_t!(ts,true);
    ...
    if t.kind == TokenKind::Hash && is_first {
        ...
        match t.kind {
            TokenKind::Scope => {
                ns.attr.insert("name".to_string(),name.clone());
                ts.flush(1);
            ...
            _ => return Err(SvError::syntax(t, "statement. Expecting scope/type".to_string()))
        }
        ...
        n.attr.insert("name".to_string(),name);
    }
    ...
    if t.kind == TokenKind::SquareLeft {
        ...
        n.attr.insert("range".to_string(),s);
        t = next_t!(ts,true);
    }
    // Check for members
    if t.kind == TokenKind::Dot {
...
pub fn parse_system_task(ts : &mut TokenStream) -> Result<AstNode, SvError> {
    ...
    match t.value.as_ref() {
        "$test" | "$value" => {
            ...
            if t.value != "$plusargs" {
                return Err(SvError::syntax(t, "system task. Expecting $plusargs".to_string()))
            }
        ...
        "$async" => {
            ...
            if t.value != "$and" && t.value != "$nand" && t.value != "$or" && t.value != "$nor" {
                return Err(SvError::syntax(t, "system task. Expecting $and/$nand/$or/$nor".to_string()))
            }
            name.push_str(&t.value);
            t = expect_t!(ts,"system task $plusargs",TokenKind::SystemTask);
            if t.value != "$array" && t.value != "$plane" {
                return Err(SvError::syntax(t, "system task. Expecting $array/$plane".to_string()))
            }
            name.push_str(&t.value);
        }
        _ => {},
...
pub fn parse_func_call(ts : &mut TokenStream, node: &mut AstNode, is_param: bool) -> Result<(), SvError> {
    ...
    let mut t = next_t!(ts,false);
    if t.kind!=TokenKind::ParenLeft {
        return Err(SvError::syntax(t,"function call. Expecting open parenthesis".to_string()));
    }
    loop {
        t = next_t!(ts,true);
        ...
        match t.kind {
            TokenKind::Dot => {
                ...
                match nt.kind {
                    ...
                    _ => return Err(SvError::syntax(t,"function arg. Expecting open parenthesis".to_string()))
                }
            },
            ...
            TokenKind::TypeIntAtom if is_param => {
                ...
                let mut node_p = AstNode::new( {AstNodeKind::Param});
                node_p.attr.insert("name".to_string(), "".to_string());
                node_p.attr.insert("type".to_string(), t.value);
                node_p.attr.insert("pos".to_string(), format!("{}",cnt));
                t = next_t!(ts,true);
                if t.kind == TokenKind::KwSigning {
                    node_p.attr.insert("signing".to_string(), t.value);
                    ts.flush(1);
                } else {
                    ts.rewind(1);
                }
            ...
            TokenKind::KwVirtual if is_param => {
                ...
                if t.kind!=TokenKind::Ident && t.kind!=TokenKind::Macro {
                    return Err(SvError::syntax(t,"function arg. Expecting port name".to_string()));
                }
                s.push_str(&t.value);
                let mut node_p = AstNode::new( {AstNodeKind::Param});
                node_p.attr.insert("name".to_string(), "".to_string());
            ...
            TokenKind::KwNull => {
                ...
                let mut node_p = AstNode::new( {AstNodeKind::Port});
                node_p.attr.insert("name".to_string(), "".to_string());
                node_p.attr.insert("value".to_string(), t.value);
                node_p.attr.insert("pos".to_string(), format!("{}",cnt));
                node.child.push(node_p);
            }
            //
...
pub fn parse_constraint(ts : &mut TokenStream, node: &mut AstNode) -> Result<(), SvError> {
    ...
    if t.kind == TokenKind::KwConstraint {
        ...
        n.attr.insert("name".to_string(),nt.value);
    }
    n.attr.insert("kind".to_string(),t.value);
    node.child.push(n);
